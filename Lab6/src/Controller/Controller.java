...
public class Controller implements IController
{
    ...
    {
        repository.setProgramList(removeCompletedPrg(repository.getProgramsList()));
    }

    public boolean oneStepForGUI() throws Exception
    {
        List<ProgramState> program = removeCompletedPrg(repository.getProgramsList());
        if(program.size()>0)
        {
            oneStepForAllPrg(program);
            removeCompletedPrg(repository.getProgramsList());
            return true;
        }
        else
        {
            executor.shutdownNow();
            repository.setProgramList(program);
            return false;
        }
    }
}
